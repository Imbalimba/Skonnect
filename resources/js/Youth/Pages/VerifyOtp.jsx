...
const VerifyOtp = () => {
  ...
  useEffect(() => {
    ...
    const checkOtpStatus = async () => {
      try {
        ...
        const result = await getOtpStatus(email, 'youth');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await verifyOtp(email, otpCode, 'youth');
  ...
  const handleResendCode = async (force = true) => {
    ...
    try {
      const result = await resendOtp(email, 'youth', force);
      ...
      <div className="youth-auth-container">
        ...
        <div className="youth-auth-card">
          <div className="youth-auth-header">
            ...
            <h1 className="youth-auth-title">Verify Your Email</h1>
            <p className="youth-auth-subtitle">We've sent a verification code to {email}</p>
          </div>
